...
// Resolve relative to THIS module so paths work on /sme/ and /sme/build/
// (page-relative paths would 404 outside the homepage).
const SCANNER_SRC       = new URL('./register-scanner.js?v=2', import.meta.url).toString();
// Vendored QR generator (self-hosted only — no CDN by policy).
// `lib/qrcode.min.js` lives at the site root — walk up from
// `js/engine/home/register.js` (3 levels) to reach it.
const QR_LIB_SRC        = new URL('../../../lib/qrcode.min.js', import.meta.url).toString();
// `content/*.json` also lives at the site root — same trick so the
// modal works identically on /, /sme/, /sme/build/.
    ...
    <div class="modal" id="dlpk-modal" hidden role="dialog" aria-labelledby="dlpk-title" aria-modal="true">
      ...
      <div class="sheet">
        ...
        <section class="step" id="dlpk-step-options">
          ...
          <div class="options">
            <button type="button" class="opt opt-primary" id="dlpk-opt-tellus">
              <span class="opt-icon">
              ...
              <span class="opt-text">
                ...
                <small>30 seconds &mdash; we&rsquo;ll come back within 24 hours</small>
              </span>
...
function wire() {
  ...
  $('dlpk-opt-tellus').addEventListener('click', function () {
    // Roy spec May 2026: 不走 "Almost there" form — redirect straight
    // to /sme/build/ where the proper intake wizard lives. Path is
    // computed relative so it works from homepage, /sme/, or anywhere.
    closeModal();
    const path = window.location.pathname;
    let target;
    if (/\/sme\/build\/?(?:index\.html)?$/.test(path)) {
      target = './';
    } else if (/\/sme\//.test(path)) {
      target = 'build/';
    } else {
      target = 'sme/build/';
    }
...
async function showCardInline(email) {
  ...
  if (!cardsDataCache) {
    try {
      const r = await fetch(new URL('cards.json', CONTENT_BASE).toString(), { cache: 'no-cache' });
      cardsDataCache = await r.json();
    } catch (e) { showStep('error'); return; }
  }
  if (!portfoliosCache) {
    try {
      const r = await fetch(new URL('content.json', CONTENT_BASE).toString(), { cache: 'no-cache' });
...
function showCustomerForm(email, name) {
  // Greeting falls back to a generic "there" / hides the email line
  // when the form is opened without a Google sign-in (the new "Tell
  // us about you" path).
  const firstName = (name && name.split(' ')[0]) || 'there';
  $('dlpk-greet-name').textContent  = firstName;
  $('dlpk-greet-email').textContent = email || '';
  const avatar = $('dlpk-avatar');
  if (signedIn && signedIn.picture) {
    avatar.src = signedIn.picture;
    avatar.hidden = false;
  } else {
    avatar.hidden = true;
  }
  ...
  showStep('form');
  // No Google data → focus the name field first; otherwise jump to
  // company (the only field Google can't pre-fill).
  const firstField = name ? 'dlpk-field-company' : 'dlpk-field-name';
