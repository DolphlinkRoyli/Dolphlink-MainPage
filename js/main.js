...
function renderStats(container, stats) {
  container.innerHTML = stats.map(s => `
    <button type="button" class="stat-card" data-stat="${escapeHtml(s.key)}"
      ...
      <span class="stat-icon-wrap"><img src="media/icon/3D/${escapeHtml(s.icon)}.webp" alt="" class="stat-icon" width="88" height="88" loading="lazy" /></span>
...
function renderPortfolios(container, items) {
  container.innerHTML = items.map(p => {
    ...
    <button type="button" class="btn-portfolio" data-portfolio="${escapeHtml(p.key)}"
            ...
            aria-haspopup="dialog">
      ${tagline}
      <span class="portfolio-icon-wrap"><img src="media/icon/3D/${escapeHtml(p.icon)}.webp" alt="" class="portfolio-icon" width="88" height="88" loading="lazy" /></span>
...
window.addEventListener('load', async function () {
  ...
  function bindCardDetail(stripSelector, panelId, cardSelector) {
    ...
    function openPanel(card) {
      ...
      if (icon) pIcon.src = `media/icon/3D/${icon}.webp`;
      pIcon.style.display = icon ? 'block' : 'none';
